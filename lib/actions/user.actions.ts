...
export async function getLoggedInUser() {
    try {
      ...
      if (!result || !result.$id) {
        console.warn('No session found or session missing $id');
        return null;
      }
      ...
      if (!user) {
        console.warn(`User document not found in database for sessionId: ${result.$id}`);
        return null;
      }
...
return parseStringify({linkToken:response.data.link_token})
  ...
  export const getBanks = async ({ userId }: getBanksProps) => {
    try {
      ...
      if (!banks.documents || banks.documents.length === 0) {
        console.log(`No banks found for userId: ${userId}`);
        return [];
      }

      return parseStringify(banks.documents);
    } catch (error) {
      console.error(`Error fetching banks for userId ${userId}:`, error)
      return [];
    }
  }
  ...
  export const getBank = async ({ documentId }: getBankProps) => {
    try {
      ...
      console.error(`Error fetching bank with documentId ${documentId}:`, error)
      return null;
    }
  }
