...
export const getAccounts = async ({ userId }: getAccountsProps) => {
  try {
    ...
    // If no banks found or getBanks errored, return empty accounts
    ...
    const accounts = await Promise.all(
      banks.map(async (bank: Bank) => {
        try {
          ...
          console.error(`Error fetching account for bank ${bank.$id}:`, bankError);
          return null;
        }
      })
    );

    // Filter out null accounts (ones that failed)
